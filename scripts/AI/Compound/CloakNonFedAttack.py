...
def CreateAI(pShip, *lpTargets, **dKeywords):
	...
	if not pAllTargetsGroup.GetNameTuple():
		pAllTargetsGroup.AddName("This ship probably wont exist")
	...
	######### AI Builder Begin #########
	pBuilderAI = App.BuilderAI_Create(pShip, "Cloak Builder", __name__)
	...
	pBuilderAI.AddAIBlock("AvoidObstacles", "BuilderCreate38")
	pBuilderAI.AddDependency("AvoidObstacles", "FleeAttackOrFollow")
	...
	pBuilderAI.AddAIBlock("Cloak", "BuilderCreate41")
	pBuilderAI.AddDependency("Cloak", "AlertLevel")
...
def BuilderCreate16(pShip, pCloseRangePriorities, sInitialTarget, dKeywords):
	...
	## Setup:
	import Custom.QBautostart.Libs.LibCloakFire
	pFireScript = apply(Custom.QBautostart.Libs.LibCloakFire.myFireScript, (sInitialTarget,), dKeywords)
...
def BuilderCreate34(pShip, pPriorityList, pAllTargetsGroup, sInitialTarget):
	...
	pSelectionPreprocess = AI.Preprocessors.SelectTarget(pAllTargetsGroup)
        ...
        # Multiple Targetting change
        pSelectionPreprocess.SetRelativeImportance(fDistance = 30.0, fInFront = 0.2, fIsTarget = -0.1, fShield = 0.2, fWeapons = 15.0, fHull = 7.5, fDamage = 1.0, fPriority = 1.0, fPopularity = -1.1)
	## The PreprocessingAI:
...
def BuilderCreate37(pShip, pCheckWarpBeforeDeath, pNoSensorsEvasive, pSelectTarget, pFollowThroughWarpFlag):
	...
	########## AI Builder End ##########
	######### AI Builder Begin #########
def BuilderCreate38(pShip, pFleeAttackOrFollow):
	########## AI Builder End ##########
	#########################################
	# Creating PreprocessingAI AvoidObstacles at (9, 512)
	## Setup:
	import AI.Preprocessors
	...
	########## AI Builder End ##########
	######### AI Builder Begin #########
def BuilderCreate39(pShip, pAvoidObstacles, dKeywords):
	########## AI Builder End ##########
	#########################################
	...
	pPowerManagement.SetContainedAI(pAvoidObstacles)
...
def BuilderCreate41(pShip, pAlertLevel):
	########## AI Builder End ##########
	#########################################
	# Creating PreprocessingAI Cloak at (7, 668)
	## Setup:
	import AI.Preprocessors
	...
	pCloak.SetContainedAI(pAlertLevel)
