...
def RandomRotation(x,y,fDist,fMinAngle,fMaxAngle,fRandom=0.0):
	import math
	if Random==0.0:
		angle=Gaussian(fMinAngle,fMaxAngle)*sign(Random(-1.0,1.0))
		return x-fDist*math.cos(angle),y-fDist*math.sin(angle)
	else:
		angle=Gaussian(fMinAngle,fMaxAngle,fRandom)*sign(Random(-1.0,1.0,fRandom))
		return x-fDist*math.cos(angle),y-fDist*math.sin(angle)
		
def Gaussian(fVal1,fVal2,fRandom=0.0):
	return (Random(fVal1,fVal2,fRandom)+Random(fVal1,fVal2,fRandom)+Random(fVal1,fVal2,fRandom)+Random(fVal1,fVal2,fRandom))/4.0
...
def debug(s):
	...
	import nt	
	debug_file=nt.open("scripts\\Custom\\AdvancedTechnologies\\Data\\Debug.txt",nt.O_WRONLY|nt.O_APPEND|nt.O_CREAT)
