...
class ShipSettings:
        ...
        def SetShields(self, ShieldNum, Value):
                ...
                if self.pShip.GetShields().IsOn() == 0 and Value > 0:
	...
	def Load(self, Switch):
        ...
        def GetWeapon(self, Weapon):
                ...
                if self.pPhasers.has_key(Weapon):
                        if (self.pShip.GetPhaserSystem().IsOn() == 0):
                                return 0.0
                        return self.pPhasers[Weapon][5]
                elif self.pPulse.has_key(Weapon):
                        if (self.pShip.GetPulseWeaponSystem().IsOn() == 0):
                                return 0.0
                        return self.pPulse[Weapon][5]
                elif self.pTorp.has_key(Weapon):
                        if (self.pShip.GetTorpedoSystem().IsOn() == 0):
                                return 0.0
...
def RefreshTimer(pObject=None, pEvent=None):
		...
		debug(__name__ + ", RefreshTimer Done")
                return 0
        if pExtendedPowerWindow.IsVisible() == 1:
                updategraphics = 1
        dTimeLeft=0.5
        pPlayer = MissionLib.GetPlayer()
        if pPlayer:
		...
		debug(__name__ + ", RefreshTimer Alive 1")
                if Ships[str(pPlayer)].forceshieldupdate == 1 and updategraphics == 1:
                        for i in range(App.ShieldClass.NUM_SHIELDS):
                                pShieldBars[i].SetValue(Ships[str(pPlayer)].ShieldStatus[i])
                                pShieldBars[i].Resize(0.2, 0.0364, 0)
                        if Ships[str(pPlayer)].forceshieldupdate == 1:
                                Ships[str(pPlayer)].oldPower = newPower
                                Ships[str(pPlayer)].forceshieldupdate = 0
        
		debug(__name__ + ", RefreshTimer Alive 2")
		...
		debug(__name__ + ", RefreshTimer Alive 3")
		debug(__name__ + ", RefreshTimer Alive 3 " + str(pExtendedPowerWindow.IsVisible()))
                if updategraphics == 1:
                        for i in range(App.ShieldClass.NUM_SHIELDS):
                                if Ships[str(pPlayer)].forceshieldupdate == 1:
                                        pShieldBars[i].SetValue(Ships[str(pPlayer)].ShieldStatus[i])
		...
		debug(__name__ + ", RefreshTimer Alive 4")
		if updategraphics == 1:
                	UpdateDynamicPowerWindowInterieur(updategraphics)
        
		debug(__name__ + ", RefreshTimer Alive 5")
                if pExtendedPowerWindow.IsVisible() and updategraphics:
                        pExtendedPowerWindow.Layout()

	debug(__name__ + ", RefreshTimer Alive 6")
        pTimerEvent = App.TGEvent_Create()
...
def WeaponFired(pObject, pEvent):
	...
	WeaponName = pFiringWeapon.GetName()
		...
		debug(__name__ + ", WeaponFired Return 1")
                return
	PowerLevel = Ships[str(pShip)].GetWeapon(WeaponName)
	debug(__name__ + ", WeaponFired GetWeapon Done")
        if not PowerLevel or PowerLevel <= 1.5:
		debug(__name__ + ", WeaponFired Return 2")
                return
        ...
        if RandNum < myLevel:
		debug(__name__ + ", WeaponFired Return 3")
                # no damage
                return
        # 2. how much damage?
	...
	debug(__name__ + ", WeaponFired End")
