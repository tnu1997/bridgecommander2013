...
def StartAbandonShip(pShip):
	...
	while (i <= CreateShipsNum):
		...
		AS_EscapePod = loadspacehelper.CreateShip(PodModel, pSet, PodName, PodLaunchPoint)
                ...
                # Disable Collisions
		debug(__name__ + ", StartAbandonShip, DisableCollisions")
                AS_EscapePod.EnableCollisionsWith(pShip, 0)
                AS_EscapePod.DisableCollisionDamage(1)
		debug(__name__ + ", StartAbandonShip, DisableCollisions END")
                i2 = i
                while (i2 >= 1 and i != 1):
			debug(__name__ + ", StartAbandonShip, EnableCollisionWith LOOP")
			debug(__name__ + ", StartAbandonShip, EnableCollisionWith LOOP With " + str(i) + " " + str(i2))
			pEnableCollisionPartner = MissionLib.GetShip("Escape Pod " + str(i2))
			debug(__name__ + ", StartAbandonShip, EnableCollisionWith " + str(pEnableCollisionPartner))
                        AS_EscapePod.EnableCollisionsWith(pEnableCollisionPartner, 0)
			debug(__name__ + ", StartAbandonShip, EnableCollisionWith DONE")
                        i2 = i2 - 1
                collisiondisable = 0
		debug(__name__ + ", StartAbandonShip, EnableCollisionWith LOOP Done")
		...
		debug(__name__ + ", StartAbandonShip, EnableCollisionWith GetLocalRandomPointAndNormalOnModel on " + str(pShip))
		debug(__name__ + ", StartAbandonShip, EnableCollisionWith GetLocalRandomPointAndNormalOnModel on " + str(pShip.GetName()))
		debug(__name__ + ", StartAbandonShip, EnableCollisionWith GetLocalRandomPointAndNormalOnModel Status: %d %d " % (pShip.IsDead(), pShip.IsDying()))
		if pShip.GetHull():
			debug(__name__ + ", StartAbandonShip, EnableCollisionWith GetLocalRandomPointAndNormalOnModel Hull: %f " % (pShip.GetHull().GetConditionPercentage()))
		...
		if pShip.IsDead() or pShip.IsDying() or not pShip.GetHull() or pShip.GetHull().GetConditionPercentage() == 0:
			debug(__name__ + ", StartAbandonShip, EnableCollisionWith GetLocalRandomPointAndNormalOnModel exiting")
			return 1
		pShip.GetLocalRandomPointAndNormalOnModel(vLocation, vNormal)
		debug(__name__ + ", StartAbandonShip, EnableCollisionWith GetRamdomPoint Done")
		vLocation.Add(pShip.GetWorldLocation())
		debug(__name__ + ", StartAbandonShip, EnableCollisionWith Add Done")
		kPoint = App.TGPoint3()
		...
		kup.Add(vNormal)"""
		debug(__name__ + ", StartAbandonShip, EnableCollisionWith Add2 Done")
		#Now Position them
		AS_EscapePod.SetTranslate(kPoint)
		debug(__name__ + ", StartAbandonShip, EnableCollisionWith Translate Done")
		...
		debug(__name__ + ", StartAbandonShip, Switch Player")
		...
		debug(__name__ + ", StartAbandonShip, Set AI.Player.FlyForward AI")
		AS_EscapePod.SetAI(AI.Player.FlyForward.CreateWithAvoid(AS_EscapePod, 10.0))

                # re-enable Collisions
		debug(__name__ + ", StartAbandonShip, ReenableCollisions")
                if collisiondisable:
                        AS_EscapePod.DisableCollisionDamage(0)
                        AS_EscapePod.EnableCollisionsWith(pShip, 1)
		debug(__name__ + ", StartAbandonShip, ReenableCollisions DONE")
		...
		# End While loop here
	debug(__name__ + ", StartAbandonShip END")
	return 1
