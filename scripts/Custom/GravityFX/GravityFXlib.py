...
################################################################
def ConvertKMtoGU(n):
	return App.UtopiaModule_ConvertKilometersToGameUnits(n)

def ConvertGUtoKM(n):
	return App.UtopiaModule_ConvertGameUnitsToKilometers(n)
...
def GetMassByRadius(radius, density = 5.515):
	...
	mass = (density*math.pow(10, 12))*vol
	return mass
...
#################################################################################################################
def Dice(number):
	return App.g_kSystemWrapper.GetRandomNumber(number)
...
def PlaySound(sFile, sName = "GravFXlib-PlaySound"):
	...
	App.g_kSoundManager.PlaySound(sName)
	return Sound
...
def GetRandomFloat():
	...
	index = 1.0
	n = num
	x = App.g_kSystemWrapper.GetRandomNumber(10)
	while n < 0.1 and x > 2:
		n = n*10
		index = index*10
		x = App.g_kSystemWrapper.GetRandomNumber(10)
...
def GetRandomInRange(start, stop=None):
	...
	istart = float(start)
	if stop is None:
		return App.g_kSystemWrapper.GetRandomNumber(start)
	istop = float(stop)
	width = istop - istart
	if width >= maxwidth:
		return float(istart + App.g_kSystemWrapper.GetRandomNumber(width))
