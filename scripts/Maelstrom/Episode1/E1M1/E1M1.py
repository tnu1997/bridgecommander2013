...
def PreLoadAssets(pMission):
	...
	loadspacehelper.PreloadShip("Sovereign", 3)
	loadspacehelper.PreloadShip("FedStarbase", 1)
	loadspacehelper.PreloadShip("DryDock", 5)
...
def Initialize(pMission):
	...
	MissionLib.SetMaxTorpsForPlayer("Photon", 250)
...
def InitializeGlobals(pMission):
	...
	g_lFriendlyShips	=	[
					"Starbase 12", "Devore", "San Francisco", "Dry Dock", "Dry Dock2", "Dry Dock3", "Drydock D", "Drydock E",
"USS Pegasus", "Soho Station", "Shuttle1", "Shuttle2", "Shuttle3", "Nightingale", "Sovereign Hull1",
							"Sovereign Hull2", "Sovereign Hull3"
							]
...
def CreateStartingObjects(pMission):
	...
	pPlayer.ReplaceTexture("data/Models/SharedTextures/Galaxy/DauntlessID_glow.tga", "defaultID_glow")
	pPlayer.ReplaceTexture("data/Models/SharedTextures/Galaxy/DauntlessID_spec.tga", "defaultID_spec")
	
	# Create dry dock installations
	...
	pDryDock4	= loadspacehelper.CreateShip("DryDock", pDryDockSet, "Drydock D", "DryDock4")
	pDryDock5	= loadspacehelper.CreateShip("DryDock", pDryDockSet, "Drydock E", "DryDock5")
	pStation	= loadspacehelper.CreateShip("SpaceFacility", pDryDockSet, "Soho Station", "StationPlacement")
	pNight		= loadspacehelper.CreateShip("NebulaVar4", pDryDockSet, "Nightingale", "DryDock2")
	...
	# Create unfitted Sovereign-class starship hulls
	pSovHull1	= loadspacehelper.CreateShip("Sovereign", pDryDockSet, "Sovereign Hull1", "DryDock3")
	pSovHull2	= loadspacehelper.CreateShip("Sovereign", pDryDockSet, "Sovereign Hull2", "Stayput1")
	pSovHull3	= loadspacehelper.CreateShip("Sovereign", pDryDockSet, "Sovereign Hull3", "Stayput2")
	
	# Create a docked Galaxy-class Dreadnought
	pGalDread	= loadspacehelper.CreateShip("GalaxyDreadnought", pDryDockSet, "USS Pegasus", "Stayput5")
	...
	for pObject in [ pShuttle1, pShuttle2, pShuttle3, pSovHull1, pSovHull2, pSovHull3]:
		pObject.SetHailable(FALSE)
	
	# Set the none moving objects static
	for pObject in [ pDryDock, pDryDock1, pDryDock2, pDryDock4, pDryDock5, pStation, pNight, pSovHull1, pSovHull2, pSovHull3, pGalDread ]:
		pObject.SetStatic(TRUE)
	
	# Shut off the lights on the Sovereign-class hulls and the Galaxy-class Dreadnought
	for pObject in [pSovHull1, pSovHull2, pSovHull3, pGalDread, pNight]:
		pObject.DisableGlowAlphaMaps()
	
	# Hide the subsystems of the Sovereign-class hulls
	for pObject in [pSovHull1, pSovHull2, pSovHull3]:
		MissionLib.HideSubsystems(pObject)
		
	# In StarbaseSet
	pStarbase	= loadspacehelper.CreateShip("FedStarbase", pStarbaseSet, "Starbase 12", "Starbase12 Location")
	pStarbase.SetAlertLevel(App.ShipClass.GREEN_ALERT)
	...
	for pObject in [pDryDock, pDryDock1, pDryDock2, pDryDock4, pDryDock5, pStation, pNight, pSovHull1, pSovHull2, pSovHull3, pGalDread]:
		pObject.SetAI(E1M1_AI_Stay.CreateAI(pObject))
...
def ObjectDying(TGObject, pEvent):
	...
	if (sSetName == "DryDock") and (sShipName != "player"):
...
def HailHandler(TGObject, pEvent):
	...
	elif (sTargetName == "USS Pegasus"):
		pData = App.g_kLocalizationManager.Load("data/TGL/Maelstrom/Episode 6/Episode6.tgl")
		pKiskaHail = App.CharacterAction_Create(g_pKiska, App.CharacterAction.AT_SAY_LINE, "E6HailAnyone4", None, 0, pData)
		App.g_kLocalizationManager.Unload(pData)
		pKiskaHail.Play()
		
	elif (sTargetName in ["Dry Dock", "Dry Dock2", "Drydock D"]) and (g_bDryDockHailed == FALSE):
		global g_bDryDockHailed
	...
	elif ((sTargetName == "Dry Dock3") or (sTargetName == "Drydock E")):
		pKiskaHail = App.CharacterAction_Create(g_pKiska, App.CharacterAction.AT_SAY_LINE, "HailOpen1", None, 0, g_pGeneralDatabase)
		pKiskaHail.Play()
		pData = App.g_kLocalizationManager.Load("data/TGL/Maelstrom/Episode 6/Episode6.tgl")
		pKiskaHail = App.CharacterAction_Create(g_pKiska, App.CharacterAction.AT_SAY_LINE, "E6HailAnyone3", None, 0, pData)
		App.g_kLocalizationManager.Unload(pData)
		pKiskaHail.Play()
	
	elif (sTargetName == "Soho Station"):
		MissionLib.CallWaiting(None, TRUE)
		pKiskaHail = App.CharacterAction_Create(g_pKiska, App.CharacterAction.AT_SAY_LINE, "HailOpen1", None, 0, g_pGeneralDatabase)
		pKiskaHail.Play()
		SecondDryDockHail()

	elif (sTargetName in [ "Dry Dock", "Dry Dock2", "Drydock D"]) and (g_bDryDockHailed == TRUE):
		MissionLib.CallWaiting(None, TRUE)
		pKiskaHail = App.CharacterAction_Create(g_pKiska, App.CharacterAction.AT_SAY_LINE, "HailOpen1", None, 0, g_pGeneralDatabase)
		pKiskaHail.Play()
		SecondDryDockHail()
...
def CreateSanFrancisco(TGObject, pEvent):
	...
	# Create the ship
	pSF	= loadspacehelper.CreateShip("SanFrancisco", pSet, "San Francisco", "SFStart", 1)
	pSF.ReplaceTexture("data/Models/SharedTextures/Galaxy/SanFranciscoID_glow.tga", "defaultID_glow")
	pSF.ReplaceTexture("data/Models/SharedTextures/Galaxy/SanFranciscoID_spec.tga", "defaultID_spec")
