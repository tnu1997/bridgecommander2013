...
def SetAffiliations(pMission):
	...
	g_pEnemies.AddName("Galor5")
	g_pEnemies.AddName("Galor6")
...
def MatanWarpsIn(pAction):
	...
	pKeldon = loadspacehelper.CreateShip("MatanKeldon2", pSet, "Matan's Keldon", "Matan Warp In", TRUE)
...
def CardEnterDialog():
	...
	pSeq.AppendAction(App.TGScriptAction_Create("MissionLib", "SetTarget", "Matan's Keldon"))
...
def SetAttackAI(pAction):
	...
	pKeldon = MissionLib.GetShip("Matan's Keldon", pSet)
...
def MatanWeaponHitHandler(pObject, pEvent):
	...
	if(pShip):
		...
		if(pSystem):
			if(pSystem.GetConditionPercentage () < 0.10):
				MissionLib.SetConditionPercentage(pSystem, 0.10)

		pSystem = pShip.GetPowerSubsystem()
		if(pSystem):
			if(pSystem.GetConditionPercentage () < 0.05):
...
def JonKaWarpDialog():
	...
	if (pBOP):
		# Bird of prey still also alive.  iWant is always 6.
		iWant = 6
	else:
		...
		if (fHullSystem < 0.25 or fPowerSystem < 0.25 or fWarpSystem < 0.25):
			iWant = 3		# Only want three ships in Belaruz2
		elif (fHullSystem < 0.4 or fPowerSystem < 0.4 or fWarpSystem < 0.4):
			iWant = 4		# Four ships in Belaruz2
		else:
			iWant = 6		# All six Galors in Belaruz2
	...
	if (iCount < iWant):	
		pGalor5 = loadspacehelper.CreateShip("Galor", pSet2, "Galor5", "Galor5 Start")
		pGalor5.SetAI(Maelstrom.Episode3.E3M5.GalorGroup2AI.CreateAI(pGalor5, "Galor5 Start"))

		iCount = iCount + 1
		
	if (iCount < iWant):
		pGalor6 = loadspacehelper.CreateShip("Galor", pSet2, "Galor6", "Galor6 Start")
		pGalor6.SetAI(Maelstrom.Episode3.E3M5.GalorGroup2AI.CreateAI(pGalor6, "Galor6 Start"))
