...
def PreLoadAssets(pMission):
	...
	loadspacehelper.PreloadShip("KessokHeavy2", 1)
...
def CreateShips():
	...
	pPlayer = MissionLib.CreatePlayerShip("Sovereign", pVesuvi6, "player", "Player Start")
	...
	pPrometheus = loadspacehelper.CreateShip("Prometheus", pStarbase, "USS Prometheus", "Circling Nebula")
	...
	pBerkeley = loadspacehelper.CreateShip("Berkeley2", pVesuvi4, "USS Berkeley", "Berkeley Start")
...
def AddNavAlpha(pAction):
	...
	if pProbe:
		pProbe.SetSplashDamage(6000.0, pProbe.GetRadius() * 20.0)
...
def CreateNightingale(pAction):
	pVesuvi4 = App.g_kSetManager.GetSet("Vesuvi4")
	pNightingale = loadspacehelper.CreateShip("NebulaVar4", pVesuvi4, "USS Nightingale", "Nightingale Arrive", TRUE)
	...
	pTractors = pNightingale.GetTractorBeamSystem()
	for iCounter in range(pTractors.GetNumChildSubsystems()):
		...
		pProp.SetMaxDamage(10000000000.0)
...
def SetupRescueBerkeley(pAction):
	...
	pSet = App.g_kSetManager.GetSet("Vesuvi4")
	pKessok = loadspacehelper.CreateShip("KessokHeavy2", pSet, "Strange Ship", "Nightingale Arrive")
	...
	if pBerkeley:
		...
		MissionLib.SetConditionPercentage(pBerkeley.GetHull(), .45)
