...
# Modified for BC Super Mod V3.0	06/13/06	Bryan Cook
###############################################################################
import App
import MissionLib
import Maelstrom
import Tactical.Projectiles.FedPhasedPlasma

# Don't tamper with the "FedPhasedPlasma" torpedo script, and for god's sake don't
# tamper with this script!	-CR
		...
		if pPlayer and (pPlayer.GetObjID() == pTorpedo.GetParentID()):
			...
			if pTorpSys:
				...
				if pAmmoType:
					...
					if pAmmoType.GetAmmoName() == Tactical.Projectiles.FedPhasedPlasma.GetName():
						...
						if iNum == 1:
							...
							lLauncherNames = ["Tube F1", "Tube F2", "Tube F3", "Tube F4", "Turret F"]
							for sLauncher in lLauncherNames:
								if (sLauncher == pLauncher.GetName()):
