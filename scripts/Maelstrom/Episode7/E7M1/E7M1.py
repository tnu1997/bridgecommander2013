...
def PreLoadAssets(pMission):
	...
	loadspacehelper.PreloadShip("Keldon", 15)
...
#				DebugPrint("Picard warps to Starbase")
				...
				pEnterprise = loadspacehelper.CreateShip( "EnterpriseE", pSet, "USS Enterprise", "Nebula Way1", 1 )
...
#	DebugPrint("Set up sequence to add phased torpedoes next time you dock with Starbase 12")
	...
	pAction = App.TGScriptAction_Create(__name__, "FixTorpMax", "Phased-Plasma", 40)
	pSequence.AppendAction(pAction)

	pAction = App.TGScriptAction_Create("MissionLib", "LoadTorpedoes", "Phased-Plasma", 40)
	pSequence.AppendAction(pAction)
...
def IsUsingQuantums():
	...
	if(pTorpSys):
		...
		if(pAmmoType):
			import Tactical.Projectiles.FedQuantum
			if pAmmoType.GetAmmoName() == Tactical.Projectiles.FedQuantum.GetName():
				return TRUE
	return FALSE
...
#	DebugPrint("Creating second wave of Cardassians")	
	...
	pKeldon7 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon7", "Attacker 4 Start", 1 )
	pKeldon8 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon8", "Attacker 5 Start", 1 )
	pKeldon9 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon9", "Attacker 6 Start", 1 )
	pKeldon10 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon10", "Attacker 7 Start", 1 )
	pKeldon11 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon11", "Attacker 8 Start", 1 )
	pKeldon12 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon12", "Attacker 9 Start", 1 )
	pKeldon13 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon13", "Attacker 10 Start", 1 )
	pKeldon14 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon14", "Attacker 11 Start", 1 )
	pKeldon15 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon15", "Attacker 12 Start", 1 )
	...
	if (iNum == 1):
		...
		pKeldon7.SetAI(Keldon2AI.CreateAI(pKeldon7))

	iNum = App.g_kSystemWrapper.GetRandomNumber(2)
	if (iNum == 1):
		pKeldon8.SetAI(KeldonAI.CreateAI(pKeldon8))
	else:
		pKeldon8.SetAI(Keldon2AI.CreateAI(pKeldon8))

	iNum = App.g_kSystemWrapper.GetRandomNumber(2)
	if (iNum == 1):
		pKeldon9.SetAI(KeldonAI.CreateAI(pKeldon9))
	else:
		pKeldon9.SetAI(Keldon2AI.CreateAI(pKeldon9))
	
	iNum = App.g_kSystemWrapper.GetRandomNumber(2)
	if (iNum == 1):
		pKeldon10.SetAI(KeldonAI.CreateAI(pKeldon10))
	else:
		pKeldon10.SetAI(Keldon2AI.CreateAI(pKeldon10))

	iNum = App.g_kSystemWrapper.GetRandomNumber(2)
	if (iNum == 1):
		pKeldon11.SetAI(KeldonAI.CreateAI(pKeldon11))
	else:
		pKeldon11.SetAI(Keldon2AI.CreateAI(pKeldon11))

	iNum = App.g_kSystemWrapper.GetRandomNumber(2)
	if (iNum == 1):
		pKeldon12.SetAI(KeldonAI.CreateAI(pKeldon12))
	else:
		pKeldon12.SetAI(Keldon2AI.CreateAI(pKeldon12))
	
	iNum = App.g_kSystemWrapper.GetRandomNumber(2)
	if (iNum == 1):
		pKeldon13.SetAI(KeldonAI.CreateAI(pKeldon13))
	else:
		pKeldon13.SetAI(Keldon2AI.CreateAI(pKeldon13))

	iNum = App.g_kSystemWrapper.GetRandomNumber(2)
	if (iNum == 1):
		pKeldon14.SetAI(KeldonAI.CreateAI(pKeldon14))
	else:
		pKeldon14.SetAI(Keldon2AI.CreateAI(pKeldon14))

	iNum = App.g_kSystemWrapper.GetRandomNumber(2)
	if (iNum == 1):
		pKeldon15.SetAI(KeldonAI.CreateAI(pKeldon15))
	else:
		pKeldon15.SetAI(Keldon2AI.CreateAI(pKeldon15))
...
#	DebugPrint("Creating Enterprise\n")	
	...
	#Create the Enterprise
	pEnt_E = loadspacehelper.CreateShip( "EnterpriseE", pSet, "USS Enterprise", "Nebula Way1", 1 )
	pEnt_E.ReplaceTexture("data/Models/SharedTextures/Sovereign/EnterpriseID_glow.tga", "defaultID_glow")
	pEnt_E.ReplaceTexture("data/Models/SharedTextures/Sovereign/EnterpriseID_specular.tga", "defaultID_specular")
...
#	DebugPrint("Adding GoToStarbase12, Starbase12Survive and RepelCardassians Goals.\n")
	...
	MissionLib.CreateTimer(ET_CREATE_GERONIMO, __name__ + ".CreateGeronimo", fStartTime + 120, 0, 0)
	...
	MissionLib.CreateTimer(ET_CREATE_ENTERPRISE, __name__ + ".CreateEnterprise", fStartTime + 240, 0, 0)

	# Create a Timer that triggers the CreateThirdWave Function
	MissionLib.CreateTimer(ET_THIRD_WAVE, __name__ + ".CreateThirdWave", fStartTime + 300, 0, 0)
