...
def PreLoadAssets(pMission):
	...
	loadspacehelper.PreloadShip("Galor", 18)
...
def CreateShips():
	...
	pRiha1 = Systems.Riha.Riha1.GetSet()
   	...
   	# Starbase 12 ships
	pStarbase = loadspacehelper.CreateShip("FedStarbase", pStarbase12Set, "Starbase 12", "Starbase12 Location")
	pPlayer = MissionLib.CreatePlayerShip("Sovereign", pStarbase12Set, "player", "Player Start")
	...
	if(not g_bCebalraiWon):
		...
		loadspacehelper.CreateShip("Galor", pCebalrai2, 
												"Galor 6", "Keldon11 Start" )
		loadspacehelper.CreateShip("Galor", pCebalrai2, 
												"Galor 7", "Keldon12 Start" )
...
def SetAffiliations(pMission):
	...
	g_pEnemies.AddName("Matan's Keldon")
	...
	g_pEnemies.AddName("GalorR6")
	g_pEnemies.AddName("GalorR7")
	...
	g_pCebalraiHostiles.AddName("Galor 6")
	g_pCebalraiHostiles.AddName("Galor 7")
	...
	if g_pNepentheHostiles:
		...
		g_pNepentheHostiles.AddName("GalorR6")
		g_pNepentheHostiles.AddName("GalorR7")
...
#		DebugPrint("%s exited set %s" % (pShip.GetName(), pEvent.GetCString()))
		...
		if not pShip.IsDead():
			...
			elif (pcSetName == "Cebalrai2") and ((pcName == "Galor 1") or (pcName == "Galor 2") or (pcName == "Galor 6") or (pcName == 				"Galor 7")):
				global g_iCebalraiEnemiesGone
				g_iCebalraiEnemiesGone = g_iCebalraiEnemiesGone + 1

				if g_iCebalraiEnemiesGone == 4:
					...
					pSeq = MissionLib.NewDialogueSequence()
					pSeq.AppendAction(Bridge.BridgeUtils.MakeCharacterLine(pFelix, "E6M1Savoy3Warp1",
																			pE6M1Database))
					MissionLib.QueueActionToPlay(pSeq)
...
def SetRiha1ShipsAI():
	...
	DamageShip(pGalor1, 0.25, 0.35)
	DamageShip(pGalor2, 0.15, 0.25)
...
def CebalraiOutpostDestroyed(pAction):
	...
	if MissionLib.GetNumObjectsAlive(g_pCebalraiHostiles):
		...
		if pGalor6:
			pWarp = pGalor6.GetWarpEngineSubsystem()
			if pWarp and not pWarp.IsDisabled():
				pGalor1.SetAI(WarpOutAI.CreateAI(pGalor6))

		pGalor7 = MissionLib.GetShip("Galor 7")
		if pGalor2:
			pWarp = pGalor7.GetWarpEngineSubsystem()
			if pWarp and not pWarp.IsDisabled():
				pGalor7.SetAI(WarpOutAI.CreateAI(pGalor7))

	pFelix = Bridge.BridgeUtils.GetBridgeCharacter("Tactical")
	pSeq = MissionLib.NewDialogueSequence()
	pSeq.AppendAction(Bridge.BridgeUtils.MakeCharacterLine(pFelix, "E4M6CebelraiOutpostDestroyed",
													g_pMissionDatabase))
	MissionLib.QueueActionToPlay(pSeq)
...
def SetCebalraiShipsAI(pAction):
	...
	pGalor6 = MissionLib.GetShip("Galor 6")
	if pGalor6:
		pGalor6.SetAI(AttackPlayerAI.CreateAI(pGalor6))

	pGalor7 = MissionLib.GetShip("Galor 7")
	if pGalor7:
		pGalor7.SetAI(AttackPlayerAI.CreateAI(pGalor7))
...
#	Return:	None
################################################################################
def ProdToCebalrai(TGObject, pEvent):
	if g_bMissionTerminated:
		pObject.CallNextHandler(pEvent)
		return
...
#	Return:	None
################################################################################
def ProdToBranch(TGObject, pEvent):
	if g_bMissionTerminated:
		pObject.CallNextHandler(pEvent)
		return
...
#	DebugPrint("Calling ChangeShipAI")
	...
	pKeldon1 = App.ShipClass_GetObject(App.SetClass_GetNull(), "Matan's Keldon")
...
#	Return:	None
################################################################################
def Galor1WarpsIn(TGObject, pEvent):
	if g_bMissionTerminated:
		pObject.CallNextHandler(pEvent)
		return
...
#	Return:	None
################################################################################
def Galor2WarpsIn(TGObject, pEvent):
	if g_bMissionTerminated:
		pObject.CallNextHandler(pEvent)
		return
...
#	Return:	None
################################################################################
def Keldon1WarpsIn(TGObject, pEvent):
	if g_bMissionTerminated:
		pObject.CallNextHandler(pEvent)
		return
...
#	DebugPrint("Keldon1WarpsIn() called.")
	...
	# Create ships
	pKeldon1 = loadspacehelper.CreateShip("MatanKeldon2", pNepenthe1, "Matan's Keldon", "Keldon1 Warp", TRUE)
	pKeldon1.SetInvincible(1)
	pKeldon1.SetCollisionsOn(0)
	pWarp = pKeldon1.GetWarpEngineSubsystem()
	pImp = pKeldon1.GetImpulseEngineSubsystem()
	for pSystem in [pWarp, pImp]:
		pSystem.SetInvincible(1)
...
#	Return:	None
################################################################################
def KessokWarpIn(TGObject, pEvent):
	if g_bMissionTerminated:
		pObject.CallNextHandler(pEvent)
		return
...
#	DebugPrint("KessokWarpIn() called.")
	...
	# Create ships
	pKessok = loadspacehelper.CreateShip("KessokLight", pNepenthe1, "Strange Ship", "KessokAmbassador", TRUE)
	pKessok.SetInvincible(1)
	pKessok.SetCollisionsOn(0)
	pWarp = pKessok.GetWarpEngineSubsystem()
	pImp = pKessok.GetImpulseEngineSubsystem()
	for pSystem in [pWarp, pImp]:
		pSystem.SetInvincible(1)
...
def RemoveDetectionChecks(pAction):
	...
	if pGalor2:
		pGalor2.RemoveHandlerForInstance(App.ET_WEAPON_HIT, __name__ + 
												".PlayerFired")
	pKeldon1 = MissionLib.GetShip("Matan's Keldon")
	if pKeldon1:
...
def Reinforcements(pAction):
	...
	pGalorR6 = loadspacehelper.CreateShip("Galor", pNepenthe1, 
										"GalorR6", "Placement 6", 1)
	pGalorR7 = loadspacehelper.CreateShip("Galor", pNepenthe1, 
										"GalorR7", "Placement 7", 1)
	...
	if pGalorR6:
		pGalorR6.SetAI(AttackPlayerAI.CreateAI(pGalorR6))
	if pGalorR7:
		pGalorR7.SetAI(AttackPlayerAI.CreateAI(pGalorR7))
...
def DamageShip (pShip, fMinPercent, fMaxPercent):
	...
	if (pSystem):
		...
		pSystem.SetConditionPercentage (r)
def MissionWon():
	MissionLib.LoadTorpedoes(None,"Quantum", 100)
