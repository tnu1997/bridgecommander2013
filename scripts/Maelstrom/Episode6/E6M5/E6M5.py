...
def PreLoadAssets(pMission):
	...
	loadspacehelper.PreloadShip("Zhukov", 1)
	...
	loadspacehelper.PreloadShip("MatanKeldon2", 1)
	loadspacehelper.PreloadShip("Galor", 6)
	loadspacehelper.PreloadShip("KessokLight", 6)
...
#	kDebugObj.Print("Initializing global variables")
	...
	global g_lCardNames
	g_lCardNames	=	[
						"Galor 1", "Galor 2", "Galor 3", "Galor 4", "Galor 5", "Galor 6", "Keldon 1", "Keldon 2", "Keldon 3", "Keldon 4", "Keldon 5", "Keldon", "Kessok 1", "Kessok 2", "Kessok 3", "Kessok 4", "Kessok 5", "Custom Keldon", "Kessok"
						]
	global g_lMoreCardNames
	g_lMoreCardNames	=	[
							"Keldon", "Custom Keldon", "Kessok"
							]
	global g_lFedNames
	g_lFedNames	=	[
					"Starbase 12", "Khitomer", "San Francisco", "Devore", "Venture", "Zhukov", "USS Magellan", "player"
					]
...
def CreateStartingObjects(pMission):
	...
	pZhukov		= loadspacehelper.CreateShip("Zhukov", pStarbaseSet, "Zhukov", "ZhukovStart")
	pWarp = pZhukov.GetWarpEngineSubsystem()
	...
	pWarp = pKhitomer.GetWarpEngineSubsystem()
	MissionLib.MakeSubsystemsInvincible(pWarp)
	
	pMagic		= loadspacehelper.CreateShip("Galaxy", pStarbaseSet, "USS Magellan", "MagellanStart")
	pMagic.ReplaceTexture("data/Models/SharedTextures/Galaxy/MagellanID_glow.tga", "defaultID_glow")
	pMagic.ReplaceTexture("data/Models/SharedTextures/Galaxy/MagellanID_spec.tga", "defaultID_spec")
	
	pWarp = pMagic.GetWarpEngineSubsystem()
	MissionLib.MakeSubsystemsInvincible(pWarp)
	...
	if (g_bDevoreDestroyed == FALSE):
		...
		pFedShip.ReplaceTexture("data/Models/SharedTextures/Akira/DevoreID_glow.tga", "akID_glow")
		pFedShip.ReplaceTexture("data/Models/SharedTextures/Akira/DevoreID_specular.tga", "akID_specular")
		pTorps = pFedShip.GetTorpedoSystem()
		pTorps.SetAmmoType(App.AT_TWO, 0)
		pWarp = pFedShip.GetWarpEngineSubsystem()
		MissionLib.MakeSubsystemsInvincible(pWarp)
	elif (g_bSFDestroyed == FALSE):
		g_sFedEscortName = "San Francisco"
		pFedShip	= loadspacehelper.CreateShip("SanFrancisco", pStarbaseSet, "San Francisco", "FedStart")
		pFedShip.ReplaceTexture("data/Models/SharedTextures/Galaxy/SanFranciscoID_glow.tga", "defaultID_glow")
		pFedShip.ReplaceTexture("data/Models/SharedTextures/Galaxy/SanFranciscoID_spec.tga", "defaultID_spec")
		pWarp = pFedShip.GetWarpEngineSubsystem()
		MissionLib.MakeSubsystemsInvincible(pWarp)
	elif (g_bVentureDestroyed == FALSE):
		g_sFedEscortName = "Venture"
		pFedShip	= loadspacehelper.CreateShip("Venture", pStarbaseSet, "Venture", "FedStart")
		pFedShip.ReplaceTexture("data/Models/SharedTextures/Galaxy/VentureID_glow.tga", "defaultID_glow")
		pFedShip.ReplaceTexture("data/Models/SharedTextures/Galaxy/VentureID_spec.tga", "defaultID_spec")
		pWarp = pFedShip.GetWarpEngineSubsystem()
		MissionLib.MakeSubsystemsInvincible(pWarp)
	...
	pMagic.SetAI(E6M5_AI_FleeFromStarbase.CreateAI(pMagic))
...
def CreateTargetGroups(pMission):
	...
	lTezle2Names	= ["Galor 1", "Galor 2", "Galor 3", "Galor 4", "Galor 6", "Keldon 1", "Keldon 4", "Keldon 5"]
	...
	lKessokNames=	["Kessok 1", "Kessok 2", "Kessok 3", "Kessok 4", "Kessok 5", "Kessok", "Kessok 8"]
	...
	g_lCardNames	=	["Galor 4", "Galor 5", "Keldon 2", "Keldon 3", "Keldon", "Custom Keldon"]
	...
	lSecondFeds	= [ "Devore", "San Francisco", "Venture", "USS Magellan" ]
...
def CreateStartingGalors():
	...
	pGalor6		= loadspacehelper.CreateShip("Galor", pTezle2Set, "Galor 6", "Galor66Start")
	pKeldon4	= loadspacehelper.CreateShip("Keldon", pTezle2Set, "Keldon 4", "Galor77Start")
	pKeldon1	= loadspacehelper.CreateShip("Keldon", pTezle2Set, "Keldon 1", "Keldon1Start")
	pKeldon5	= loadspacehelper.CreateShip("Keldon", pTezle2Set, "Keldon 5", "Galor88Start")
	
	#make them invincible util the user arrives
	pGalor3.SetInvincible(1)
	pGalor4.SetInvincible(1)
	pGalor6.SetInvincible(1)
	pKeldon4.SetInvincible(1)
	pKeldon1.SetInvincible(1)
	pKeldon5.SetInvincible(1)
...
def StrikeForceDeparts(TGObject, pEvent):
	...
	pMagic		= App.ShipClass_GetObject(pSet, "USS Magellan")
	...
	pMagic.SetAI(E6M5_AI_Fed.CreateAI(pMagic))
	
	MissionLib.AddCommandableShip("Zhukov")
	MissionLib.AddCommandableShip("USS Magellan")
	MissionLib.AddCommandableShip(g_sFedEscortName)
	
	# Damage the shields on the Zhukov
	pShields = pZhukov.GetShields()
	pShields.SetCurShields(pShields.FRONT_SHIELDS, pShields.GetMaxShields(pShields.FRONT_SHIELDS) / 1.2)
	pShields.SetCurShields(pShields.LEFT_SHIELDS, pShields.GetMaxShields(pShields.LEFT_SHIELDS) / 1.1)
	pZhukov.DamageSystem(pZhukov.GetHull(), 1500)
	...
	pGalor6		= App.ShipClass_GetObject(pTezleSet, "Galor 6")
	pKeldon4	= App.ShipClass_GetObject(pTezleSet, "Keldon 4")
	pKeldon1	= App.ShipClass_GetObject(pTezleSet, "Keldon 1")
	pKeldon5	= App.ShipClass_GetObject(pTezleSet, "Keldon 5")
	...
	pGalor6.SetAI(E6M5_AI_Galor3.CreateAI(pGalor6, g_pCardTargets))
	pKeldon1.SetAI(E6M5_AI_Keldon1.CreateAI(pKeldon1))
	pKeldon4.SetAI(E6M5_AI_Keldon1.CreateAI(pKeldon4))
	pKeldon5.SetAI(E6M5_AI_Keldon1.CreateAI(pKeldon5))
...
def PlayerArrivesTezle2():
	...
	pGalor6 = App.ShipClass_GetObject(None, "Galor 6")
	if (pGalor6 != None):
		pGalor6.SetInvincible(0)
	pKeldon1 = App.ShipClass_GetObject(None, "Keldon 1")
	if (pKeldon1 != None):
		pKeldon1.SetInvincible(0)
	pKeldon4 = App.ShipClass_GetObject(None, "Keldon 4")
	if (pKeldon4 != None):
		pKeldon4.SetInvincible(0)
	pKeldon5 = App.ShipClass_GetObject(None, "Keldon 5")
	if (pKeldon5 != None):
		pKeldon5.SetInvincible(0)
...
def CreateLastWave(TGObject, pEvent):
	...
	pSet = App.g_kSetManager.GetSet("Tezle1")
	# Do the placements
	pKessok6 = loadspacehelper.CreateShip("KessokLight", pSet, "Kessok", "Card1Enter")
	pKessok7 = loadspacehelper.CreateShip("MatanKeldon2", pSet, "Custom Keldon", "Card2Enter")
	pKeldon4 = loadspacehelper.CreateShip("Keldon", pSet, "Keldon", "Card3Enter")
	
	pTorps = pKessok7.GetTorpedoSystem()
	pTorps.SetAmmoType(App.AT_TWO, 0)
...
#		kDebugObj.Print ("resetting khitomer AI")
	...
	# Let the USS Magellan leave if it's still alive
	pShip = App.ShipClass_GetObject(pSet, "USS Magellan")
	if (pShip != None):
		pShip.SetAI(E6M5_AI_Fed_WarpToBeol.CreateAI(pShip, "MagEnter"))
