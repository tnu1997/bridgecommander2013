...
def HailableChange(pObject, pEvent):
	...
	pObject.CallNextHandler(pEvent)
	debug(__name__ + ", HailableChange End")
...
def PostWarpEnableMenu(pAction = None):
	...
	pPlayer = App.Game_GetCurrentPlayer()
	if pPlayer:
		debug(__name__ + ", PostWarpEnableMenu SetTarget")
...
#	debug("Orbit planet.")
	...
	if pPlanet:
		...
		if pPlayer:
			...
			debug(__name__ + ", OrbitPlanet SetTarget")
...
def SetNavPointTarget(pNavMenu, pEvent):
	...
	if pNavPoint:
		pPlayer = App.Game_GetCurrentPlayer()
		if pPlayer:
			debug(__name__ + ", SetNavPointTarget SetTarget")
